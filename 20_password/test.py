...
def test_bad_num():
    ...
    bad = random_string()
    flag = "-w" if random.choice([0, 1]) else "--num"
...
def test_bad_num_words():
    ...
    bad = random_string()
    flag = "-m" if random.choice([0, 1]) else "--num_words"
    rv, out = getstatusoutput(f"{prg} {flag} {bad} {words}")
    assert rv != 0
    assert re.search(f"invalid int value: '{bad}'", out)
...
def test_num():
    ...
    rv, out = getstatusoutput(f"{prg} -s 1 -n1 {words}")
    assert rv == 0
...
def test_num_words():
    ...
    assert rv == 0
    assert out.strip() == "\n".join(
        ["DuniteBoon", "LociDefat", "WegaTitmalUnplatSatire", "IdeanClipsVitiArriet"]
    )
...
def test_133t():
    ...
    rv, out = getstatusoutput(f"{prg} -s 1 -1 {words}")
    assert rv == 0
    assert out.strip() == "\n".join(
